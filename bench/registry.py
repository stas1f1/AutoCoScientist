...
from bench.utils import get_module_dir, get_repo_dir

DEFAULT_DATA_DIR = (Path(user_cache_dir()) / "autods-bench" / "data").resolve()


class AutoDSRegistry(Registry):
    ...
    def get_lite_competition_ids(self) -> list[str]:
        ...
        lite_competitions_file = self.get_splits_dir() / "lite.txt"
    ...
    def set_data_dir(self, new_data_dir: Path) -> "Registry":
        ...
        print(f"Setting data directory to {new_data_dir}")
        return AutoDSRegistry(new_data_dir)
